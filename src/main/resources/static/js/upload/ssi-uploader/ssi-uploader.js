...
//----------------------------UPLOADFILES------------------------------------
    ...
    Ssi_upload.prototype.uploadFiles = function () {// upload the pending files
        ...
        function ajaxLoopRequest(formData, ii) {
            ...
            var ajaxOptions = $.extend({}, {//store the request to the uploadList variable
                ...
                type: 'GET',
                data:{
                  filename: thisS.toUpload[ii].name
                },
                //dong: 为了适应自己系统做的修改-end
            ...
            $.ajax(ajaxOptions).done(function (responseData, textStatus, jqXHR) {
                ...
                if (responseData.success) {
                    ...
                    var file = _formData.get('files[]')
                    _formData.delete('files[]')
                    _formData.append('OSSAccessKeyId', responseData.accessKeyId)
                    _formData.append('policy', responseData.policy)
                    _formData.append('signature', responseData.signature)
                    _formData.append('key', responseData.filename)
    ...
    var locale = {
        en: {
            ...
            upload: '上传',
            clear: '清空',
            drag: '支持【拖拽➹批量上传】 note：上传后的文件是不能删除的',
