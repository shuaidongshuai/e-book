    // MP3播放器及歌曲库js http://www.jq22.com/jquery-info19022
	...
	//添加播放歌曲
	function addMusic1(self){
        // var title = $(this).parent("li").find("a:first-child").text()
        var fileUrl = self.attr("fileUrl")
        var coverUrl = self.attr("coverUrl")
        var name = self.attr("name")
        var singer = self.attr("singer")
        myPlaylist.add({
            title: name,
            artist: singer,
            //free:true,
	...
	$("#popularMusic li").find("a.playIcon").click(function() {
        addMusic1($(this))
	});

	$("#englishMusic li").find("a.playIcon").click(function() {
        addMusic1($(this))
    });

	$("#douyinMusic li").find("a.playIcon").click(function() {
        addMusic1($(this))
    });

	function addMusic2(self){
        var title = self.find("strong").text()
        var artist = self.find("font:first-child").text()
        var mp3 = self.attr("fileUrl")
        var poster = self.find("img")[0].src
        myPlaylist.add({
            title: title,
            artist: artist,
            //free:true,
